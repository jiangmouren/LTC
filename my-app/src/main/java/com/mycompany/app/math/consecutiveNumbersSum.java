 ...
 * N/K>=k/2+1/2, if k%2==0 -> N/K>=(K+1)/2
 * N/K>=(K-1)/2+1, if k%2==1 -> N/K>=(K+1)/2
 *
 ...
 *
 * 从另外一个角度看，往左找到最左就是1，那么对应额右侧点x要满足：x+1 = 2*(N/k)
 * x = (2N/k)-1
 * 只要K>=2，那么2N/k<=N，所以(2N/K)-1<=N-1
 * 所以右侧一定满足。
 * 看左侧，不管K是奇数还是偶数，N/K都最起码比K/2大0.5(奇: N/K>=(k-1)/2 + 1; 偶: N/K>=K/2 + 1/2)
...
public class consecutiveNumbersSum {
    public int consecutiveNumbersSum(int N) {
        ...
        for(int i=2; i*(i+1)<=2*N; i++){//或者用二次方程求根公式写出解析解，或者直接用不等式控制i就可以
            if(i%2==0){
                if(N%i==(i/2)){//因为要求N/i = x + 1/2 -> N = xi+i/2 -> N%i = i/2
                    result++;
                }
            }
            else{
