...
public class MyCalendarI {
    ...
    //这个题的核心是意识到treemap里面存的这个interval tree，各个interval之间是没有overlap的
    public boolean book(int start, int end) {
        //注意这里pre & next都要用Integer而不是int，因为有可能是Null
        Integer pre = this.map.floorKey(start);
        Integer next = this.map.ceilingKey(start);
        if((pre==null || this.map.get(pre)<=start) && (next==null || next>=end)){
            map.put(start, end);
            return true;
        }
        return false;
